...
import { getOpenState } from '../data/content';
...
export default function OpenStatus({ compact = false }) {
  ...
  const label = state.open
    ? `${t('open.now')} — ${t('open.until', { t: state.until })}`
    : state.nextDay
      ? `${t('open.closed')} — ${t('open.opensDay', { d: state.nextDay === 'tomorrow' ? t('open.tomorrow') : state.nextDay, t: state.next })}`
      : state.next
        ? `${t('open.closed')} — ${t('open.opens', { t: state.next })}`
        : t('open.closed');
